...
function PredictionResult({ prediction }) {
  ...
  return (
    <div className="flex flex-col gap-2 bg-zinc-50 rounded-xl p-3">
      <div className="flex items-center gap-3">
        <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${isBedtime ? 'bg-indigo-100' : 'bg-amber-100'}`}>
          <Moon size={16} className={isBedtime ? 'text-indigo-600' : 'text-amber-500'} />
        </div>
        <div>
          <p className="text-xs text-zinc-400">{t(`sleepAssistant.types.${prediction.type}`)}</p>
          <p className="text-lg font-semibold text-zinc-800">{formatTime(prediction.time)}</p>
        </div>
      </div>
      <p className="text-xs text-zinc-400">{t('sleepAssistant.norm', { label: prediction.normLabel })}</p>
      <p className="text-xs text-zinc-500 leading-relaxed border-t border-zinc-100 pt-2">{prediction.reason}</p>
    </div>
  );
}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-100 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-zinc-700">{t('aiAssistant.title')}</span>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="flex flex-col gap-3 p-4 overflow-y-auto flex-1">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 active:scale-95 transition-all disabled:opacity-60"
          >
            <Moon size={16} />
          ...
          {/* Chips */}
          <div>
            <p className="text-xs text-zinc-400 mb-2">{t('aiAssistant.quickQuestions')}</p>
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
              {CHIPS.map(({ key, prompt }) => (
                <button
                  key={key}
                  onClick={() => handleChip(prompt)}
                  className="shrink-0 px-3 py-1.5 rounded-full bg-zinc-100 text-zinc-700 text-xs font-medium hover:bg-indigo-50 hover:text-indigo-700 active:scale-95 transition-all"
                >
          ...
          {activeContent === 'chat' && (
            <div className="bg-zinc-50 rounded-xl p-3 min-h-[48px]">
              {isChatLoading && (
              ...
              {chatAnswer && <p className="text-sm text-zinc-700 leading-relaxed">{chatAnswer}</p>}
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-t border-zinc-100 shrink-0">
          <input
            ...
            className="flex-1 text-sm bg-zinc-100 rounded-full px-4 py-2 outline-none focus:ring-2 focus:ring-indigo-300 placeholder:text-zinc-400"
          />
          <button
            onClick={handleSend}
            aria-label={t('aiAssistant.send')}
            disabled={!inputValue.trim() || isChatLoading}
            className="w-9 h-9 rounded-full bg-indigo-600 text-white flex items-center justify-center hover:bg-indigo-700 active:scale-95 transition-all disabled:opacity-40"
          >
            <Send size={15} />
          </button>
        </div>
